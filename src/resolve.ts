...
function isTypeIdentifier(type: Type): boolean {
  ...
  return (
    ...
    kindName !== "MappedType" &&
    (type.isAnonymous() === false || typeof type.getAliasSymbol() !== "undefined")
  );
}
...
export function resolve(
  ...
  if (type.isClassOrInterface() || type.isObject()) {
    ...
    if (helperName === "Partial" || helperName === "Omit" || helperName === "Pick" || helperName === "Promise") {
      ...
      switch (helperName) {
        ...
        case "Pick":
          const args = typeArguments[1].isUnion()
            ? typeArguments[1]
                .getUnionTypes()
                .map((t) => capitalizeFirstLetter(String(t.getAliasSymbol()?.getName() ?? t.getLiteralValue())))
      ...
      (type.getAliasTypeArguments().length === 1 || type.getTypeArguments().length === 1) &&
      isTypeIdentifier(type.getTypeArguments()[0] ?? type.getAliasTypeArguments()[0])
    ) {
      ...
      (type.getAliasTypeArguments().length === 1 || type.getTypeArguments().length === 1) &&
      (type.getTypeArguments()[0] ?? type.getAliasTypeArguments()[0])?.isAnonymous() === true
    ) {
      ...
      (type.getAliasTypeArguments().length === 1 || type.getTypeArguments().length === 1) &&
      (type.getTypeArguments()[0] ?? type.getAliasTypeArguments()[0])?.isUnion() === true
    ) {
      ...
      (type.getAliasTypeArguments().length === 1 || type.getTypeArguments().length === 1) &&
      (type.getTypeArguments()[0] ?? type.getAliasTypeArguments()[0])?.isIntersection() === true
    ) {
...
function resolveProperties(type: Type, spec: OpenAPIV3.Document): ResolvePropertiesReturnType {
  const result: ResolvePropertiesReturnType = type.getProperties().reduce(
    (schema, property) => {
      ...
      const modifierFlags =
        property.getValueDeclaration()?.getCombinedModifierFlags() ?? node.getCombinedModifierFlags();
      ...
      const isReadonly =
        ...
        (hasFlags(SymbolFlags.GetAccessor) === true && hasFlags(SymbolFlags.SetAccessor) === false) ||
  ...
  if (
    ...
    result.additionalProperties = resolve(stringIndexType ?? numberIndexType!, spec);
  } else {
    ...
    if (typeSymbol?.getName() === "__type" || typeText.includes("Record<")) {
      ...
      if (apparentType && apparentType !== type) {
        ...
        if (
          ...
          result.additionalProperties = resolve(apparentStringIndexType ?? apparentNumberIndexType!, spec);
        }
      }
    }
  }
...
function hasInterfaceInheritance(type: Type): boolean {
  ...
  return declarations.some((decl) => Node.isInterfaceDeclaration(decl) && decl.getExtends().length > 0);
}
...
function getOwnInterfaceProperties(type: Type, spec: OpenAPIV3.Document): ResolvePropertiesReturnType {
  ...
  const declarations = symbol.getDeclarations();
  const interfaceDecl = declarations.find((decl) => Node.isInterfaceDeclaration(decl)) as
    | InterfaceDeclaration
    | undefined;
...
export function appendJsDocTags(
  ...
  const numericTags = ["minimum", "maximum", "minLength", "maxLength", "minItems", "maxItems"];
