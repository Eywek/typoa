import { Route, Get, Post, Put, Query, Body, Header, Controller } from '../../../src'
...
export class NullableController extends Controller {
  ...
  queryNullUndefined (
    @Query('filter') filter: string | null | undefined
  ...
  @Get('header-null')
  headerNull (
    @Header('x-custom') custom: string | null
  ): { custom: string | null } {
    return { custom }
  }
  ...
  @Put('body-undefined')
  bodyUndefined (
    @Body() body: CreateUserDto | undefined
  ): { received: boolean } {
    return { received: body !== undefined }
  }
}
