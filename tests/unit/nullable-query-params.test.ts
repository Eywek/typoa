...
describe('Nullable query parameters', () => {
  const openapiFile = path.resolve(
    __dirname,
    'generated-openapi-nullable-query-params.json'
  )
  const routerFile = path.resolve(
    __dirname,
    'generated-router-nullable-query-params.ts'
  )
  ...
  it('should handle nullable query parameters correctly', async () => {
    await generate({
      ...
      controllers: [
        path.resolve(
          __dirname,
          '../fixtures/controllers/nullable-query-params.controller.ts'
        )
      ],
      openapi: {
    ...
    const nullableAndUndefinedPath =
      spec.paths['/nullable-query-params/nullable-and-undefined']?.get
