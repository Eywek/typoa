...
test('Should apply @Produces decorator correctly', async () => {
  const openapiFile = path.resolve(
    __dirname,
    'generated-openapi-controller-produces-test.json'
  )
  const routerFile = path.resolve(
    __dirname,
    'generated-router-openapi-controller-produces.ts'
  )
  await generate({
  ...
  const defaultResponse =
    spec.paths['/api/produces/default'].get.responses['200']
  ...
  const infoResponse =
    spec.paths['/api/text-controller/info'].get.responses['200']
  ...
  const detailsResponse =
    spec.paths['/api/text-controller/details'].get.responses['200']
  ...
  const overrideResponse =
    spec.paths['/api/text-controller/json-override'].get.responses['200']
...
test('Should handle @Produces with request body correctly', async () => {
  const openapiFile = path.resolve(
    __dirname,
    'generated-openapi-controller-produces-body-test.json'
  )
  const routerFile = path.resolve(
    __dirname,
    'generated-router-openapi-controller-produces-body.ts'
  )
