...
test('Should apply controller-level responses correctly', async () => {
  const openapiFile = path.resolve(
    __dirname,
    'generated-openapi-controller-response-test.json'
  )
  const routerFile = path.resolve(
    __dirname,
    'generated-router-controller-response.ts'
  )
  ...
  assert.ok(
    spec.paths['/api/users'].get.responses['400'],
    'List users should have 400 response from controller'
  )
  assert.ok(
    spec.paths['/api/users'].get.responses['403'],
    'List users should have 403 response from controller'
  )
  assert.ok(
    spec.paths['/api/users'].get.responses['404'],
    'List users should have 404 response from controller'
  )

  assert.ok(
    spec.paths['/api/users/{id}'].get.responses['400'],
    'Get user should have 400 response from controller'
  )
  assert.ok(
    spec.paths['/api/users/{id}'].get.responses['403'],
    'Get user should have 403 response from controller'
  )
  assert.ok(
    spec.paths['/api/users/{id}'].get.responses['404'],
    'Get user should have 404 response from controller'
  )

  // Post method overrides 404 response
  assert.ok(
    spec.paths['/api/users'].post.responses['404'],
    'Create user should have 404 response'
  )
  assert.strictEqual(
  ...
  assert.ok(
    spec.paths['/api/users/{id}'].put.responses['409'],
    'Update user should have 409 response'
  )
  assert.strictEqual(
  ...
  assert.ok(
    spec.paths['/api/products'].get.responses['404'],
    'List products should have 404 response'
  )
  assert.strictEqual(
  ...
  assert.ok(
    !spec.paths['/api/products'].post.responses['404'],
    'Create product should not have 404 response'
  )
  assert.ok(
    !spec.paths['/api/products'].post.responses['400'],
    'Create product should not have 400 response'
  )
  ...
  assert.ok(
    spec.paths['/api/admin'].get.responses['401'],
    'Get admin info should have 401 response'
  )
  assert.ok(
    spec.paths['/api/admin'].get.responses['403'],
    'Get admin info should have 403 response'
  )
  ...
  assert.ok(
    spec.paths['/api/admin'].post.responses['429'],
    'Create admin resource should have 429 response'
  )
  assert.strictEqual(
