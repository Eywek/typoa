...
test('Should generate openapi definition', async () => {
  const openapiFile = path.resolve(
    __dirname,
    'generated-openapi-test-valid.json'
  )
  const routerFile = path.resolve(
    __dirname,
    'generated-router-openapi-test-valid.ts'
  )
...
test('Should generate both json and yaml definitions using array of file paths', async () => {
  ...
  const routerFile = path.resolve(
    __dirname,
    'generated-router-openapi-array-test.ts'
  )
...
test('Should fail with a missing parameter decorator', async () => {
  await assert.rejects(
    () =>
      generate({
        ...
        controllers: [
          path.resolve(
            __dirname,
            '../fixtures/validation/invalid-controller.ts'
          )
        ],
        openapi: {
    ...
    {
      message:
        'Parameter MyController.get.invalidParameter must have a decorator.'
    }
  )
})
...
test('Should generate the right definition with response object', async () => {
  const openapiFile = path.resolve(
    __dirname,
    'generated-openapi-test-object-response.json'
  )
  const routerFile = path.resolve(
    __dirname,
    'generated-router-openapi-test-object-response.ts'
  )
  ...
  assert.ok(
    spec.components.schemas.SuccessResponse_Array_Entity,
    'Should have SuccessResponse_Array_Entity schema'
  )
  assert.ok(
    spec.components.schemas.SuccessResponse_Entity,
    'Should have SuccessResponse_Entity schema'
  )
  assert.ok(
    spec.components.schemas.SuccessResponse_entity_Entity_count_number,
    'Should have SuccessResponse_entity_Entity_count_number schema'
  )

  // Verify the endpoints reference different schemas (not the same type for get and post)
  const listResponse =
    spec.paths['/my-3nd-controller'].get.responses['200'].content[
      'application/json'
    ].schema
  const createResponse =
    spec.paths['/my-3nd-controller'].post.responses['200'].content[
      'application/json'
    ].schema
  assert.notStrictEqual(
    listResponse.$ref,
    createResponse.$ref,
    'GET and POST should have different response schemas'
  )
})
